...
public class EnvironmentDomainResourceTest extends JerseyTest {
	
	public EnvironmentDomainResourceTest() {
	...
	public void testGetDomainFile() throws URISyntaxException {
		WebResource resource = resource();				
		ClientResponse response = resource.path("testClientId").path("env").path("domain").get(ClientResponse.class);
	...
	public void testDeleteDomainFile() {
		WebResource resource = resource();
		ClientResponse response = resource.path("testClientId").path("env").path("domain").delete(ClientResponse.class);
	...
	public void testPostDomainFile() throws URISyntaxException {
		...
		FormDataMultiPart environmentDomainForm = new FormDataMultiPart();
		environmentDomainForm.bodyPart(new FileDataBodyPart("file", environmentDomainFile, MediaType.valueOf(MediaType.TEXT_PLAIN)));
		ClientResponse response = resource.path("testClientId").path("env").path("domain").type(MediaType.MULTIPART_FORM_DATA_TYPE).post(ClientResponse.class, environmentDomainForm);
	...
	public void testPutProblemFile() throws URISyntaxException {
		...
		FormDataMultiPart environmentDomainForm = new FormDataMultiPart();
		environmentDomainForm.bodyPart(new FileDataBodyPart("file", environmentDomainFile, MediaType.valueOf(MediaType.TEXT_PLAIN)));
		ClientResponse response = resource.path("testClientId").path("env").path("domain").type(MediaType.MULTIPART_FORM_DATA_TYPE).put(ClientResponse.class, environmentDomainForm);
