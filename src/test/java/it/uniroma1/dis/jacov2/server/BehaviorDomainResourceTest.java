...
public class BehaviorDomainResourceTest extends JerseyTest {
	...
	public void testPostDomainFile() throws URISyntaxException {
		...
		WebResource resource = resource();
		ClientResponse createBehavior = resource.path("testClientId").path("behaviors").queryParam("name", "behavior2").post(ClientResponse.class);
		ClientResponse response = resource.path("testClientId").path("behaviors").path("behavior2").path("domain").type(MediaType.MULTIPART_FORM_DATA_TYPE).post(ClientResponse.class, form);
