...
public class TargetDomainResourceTest extends JerseyTest {

	public TargetDomainResourceTest() {
	...
	public void prepareServerForTests() throws URISyntaxException {
		...
		// Delete the target already present on the server
	...
	@Test
	public void testGetDomainFile() throws URISyntaxException {
		WebResource resource = resource();				
		ClientResponse response = resource.path("testClientId").path("target").path("domain").get(ClientResponse.class);
	...
	public void testDeleteDomainFile() {
		WebResource resource = resource();
		ClientResponse response = resource.path("testClientId").path("target").path("domain").delete(ClientResponse.class);
	...
	public void testPostDomainFile() throws URISyntaxException {
		...
		FormDataMultiPart targetDomainForm = new FormDataMultiPart();
		targetDomainForm.bodyPart(new FileDataBodyPart("file", targetDomainFile, MediaType.valueOf(MediaType.TEXT_PLAIN)));
		ClientResponse response = resource.path("testClientId").path("target").path("domain").type(MediaType.MULTIPART_FORM_DATA_TYPE).post(ClientResponse.class, targetDomainForm);
	...
	public void testPutProblemFile() throws URISyntaxException {
		...
		FormDataMultiPart targetDomainForm = new FormDataMultiPart();
		targetDomainForm.bodyPart(new FileDataBodyPart("file", targetDomainFile, MediaType.valueOf(MediaType.TEXT_PLAIN)));
		ClientResponse response = resource.path("testClientId").path("target").path("domain").type(MediaType.MULTIPART_FORM_DATA_TYPE).put(ClientResponse.class, targetDomainForm);
